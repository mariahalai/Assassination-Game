...
Understanding speed patterns helps identify periods of transit (when the user is moving) versus stationary periods. We’ll plot speed over time for each day to see if there are recurring patterns.
```{r}
...
- Calculate daily total movement time and total stationary time to get a sense of typical daily transit patterns.

2. Find Stationary Periods of 2+ Minutes:
- Identify any stationary periods lasting 2 minutes or more as "safe zones."
```{r}
library(jsonlite)
library(dplyr)
library(tidyr)
...
To further analyze patterns, let’s visualize the distribution of transit and stationary times.
```{r}
...
Action: Run the following code to get average start and end times for each transit period.
```{r}
...
Action: Run the following code to calculate and visualize the relationship between speed and transit duration.
...
Goal: Pinpoint locations where stationary periods frequently occur, creating “safe zones” to avoid for bombing attempts.

Action: Use the following code to find locations (latitude and longitude) where stationary periods of 2+ minutes frequently occur.
```{r}
library(dplyr)
...
```
Step 1: Calculate Most Frequent Start Times for Movement
